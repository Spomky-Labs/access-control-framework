...
The quality tools run in the `ghcr.io/spomky-labs/phpqa` image, which [Castor](https://castor.jolicode.com/) fetches for
you. Nothing else is needed besides Docker.

* install dependencies: `castor qa:install`
* run tests: `castor qa:phpunit`
* run every check the CI runs: `castor qa:all`
* fix what can be fixed before opening a PR: `castor qa:prepare-pr`
