...
1. **Créer les trois dépôts GitHub** : `spomky-labs/access-control-framework`, puis `access-control-lib` et `access-control-bundle` en read-only. Poser le secret `GITSPLIT_TOKEN`. C'est le seul point qui reste, et rien ne peut être poussé avant.
2. ~~**Brancher la CI**~~ **fait le 2026-08-17**. Le `reusable-ci.yml` de `spomky-labs/phpqa` convient tel quel : il appelle `castor qa:*` dans l'image, et l'image fournit castor. Ce qui manquait, c'étaient les tâches. `castor.php` est désormais **autonome**, sur le patron de webauthn-framework, sans lien vers un dépôt frère. `.phpqa-config.php` est supprimé, plus personne ne le lisait. Les neuf tâches que la CI appelle passent au vert en local. Infection est coupé, voir plus bas.
...
## Le castor est autonome, et pourquoi

L'import `__DIR__ . '/../phpqa/.castor/phpqa.php'` visait un dépôt frère qu'`actions/checkout` ne ramène pas : mesuré, castor échouait avant toute tâche, donc les six jobs tombaient. La réponse n'est pas de rapatrier ce dépôt en CI mais de ne rien lui demander : **l'image `ghcr.io/spomky-labs/phpqa` fournit castor et tous les outils**, les tâches vivent dans le dépôt. C'est ce que fait webauthn-framework, et `reusable-ci.yml` n'a pas besoin d'être touché, il appelle `castor qa:*` depuis la racine.

`castor.php` porte donc les tâches en clair, sous le namespace `qa` que la CI appelle, et `phpqa()` passe par l'image quand on est dehors, directement quand on est dedans, ce qui est le cas des jobs. Vérifié en copiant le fichier seul dans un répertoire sans dépôt frère : les 18 tâches se chargent.
