...
final class ChannelListenerTest extends TestCase
{
    ...
    {
        static::assertSame(['onKernelRequest', 9], ChannelListener::getSubscribedEvents()[KernelEvents::REQUEST]);
    }
    ...
    public function testAnInsecureRequestOnAnHttpsRuleIsRedirected()
    {
        ...
        static::assertInstanceOf(RedirectResponse::class, $response = $event->getResponse());
        static::assertSame('https://localhost/secure/area', $response->getTargetUrl());
    ...
    public function testASecureRequestOnAnHttpRuleIsRedirected()
    {
        ...
        static::assertSame('http://localhost/plain', $event->getResponse()->getTargetUrl());
    }

    public function testARequestAlreadyOnTheRightChannelIsLeftAlone()
    {
        static::assertNull($this->handle('https://localhost/secure/area', 'https')->getResponse());
        static::assertNull($this->handle('http://localhost/plain', 'http')->getResponse());
    }

    public function testARuleWithoutAChannelIsLeftAlone()
    {
        static::assertNull($this->handle('http://localhost/anything', null)->getResponse());
    }
    ...
    public function testTheQueryStringSurvivesTheRedirection()
    {
        ...
        static::assertSame('https://localhost/secure?page=2&sort=name', $event->getResponse()->getTargetUrl());
    }
    ...
    public function testNonStandardPortsAreCarriedOver()
    {
        ...
        new ChannelListener($map, null, 8080, 8443)
            ->onKernelRequest($event);

        static::assertSame('https://localhost:8443/secure', $event->getResponse()->getTargetUrl());
    }
    ...
    {
        static::assertSame(1, new ReflectionMethod(ChannelListener::class, '__construct')->getNumberOfRequiredParameters());
    }
    ...
    {
        return new RequestEvent(static::createStub(HttpKernelInterface::class), Request::create($uri), $type);
    }
}
